# [中文](../../zh/player/Spells.md) | [English](../../en/player/Spells.md)

# 法术一览

> [← 返回首页](../Home.md) · 上一篇：[施法系统](Spellcasting.md) · 下一篇：[仪式吟唱](Ritual-Chants.md)
...
| **Ictus** 疾风 | ictus · gust · wind · push · shove · strike | 风 | 3 | 1.5s | 6 格锥形击退 |

## 仪式法术（念触发词进入吟唱，逐句咏唱后施放）

每个仪式法术有 **3 种咒语变体（英文/中文/日文）× 3 句**，第一句锁定的变体即本次吟唱使用；最后一句总是法术的触发词。详见[仪式吟唱](Ritual-Chants.md)。
...
| **Umbra** 幻影 | umbra · veil · 幻影 | 幻术 | 30 | 12s | — | 隐身 + 速度 II 各 20 秒 |
| **Ventus** 狂风 | ventus · gale · 狂风 | 风 | 25 | 10s | — | 8 格锥形强击退 |

> 中文/日文变体的每句咒语都带有拼音/罗马音别名——用 **IPA 引擎**时直接用普通话/日语念即可被识别；用 **Vosk 引擎**时同样可以念拼音/罗马音（Vosk 按拉丁字母转写匹配）。

## 触发词可以整句夹带

匹配器支持整词包含：说 "cast ignis now" 与单独说 "ignis" 同样有效（多词触发词如 "explosion magic" 出现在语句中记 0.95 分）。含糊发音会走模糊匹配，得分低于阈值时不会误触发。

## 自定义法术

服务器管理员可以通过**数据包**添加/覆盖法术（不需要写代码），见管理员文档[数据包法术](../admin/Datapack-Spells.md)。
